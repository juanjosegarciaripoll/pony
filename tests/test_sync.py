...
class MirrorFlagWriteThroughTestCase(unittest.TestCase):
    """Flags a sync learns from the server must reach the mirror too.

    Otherwise a Maildir or mbox tree shared with another client shows
    everything as unread no matter what the server said — the reason
    ``MirrorRepository.set_flags`` existed with no caller.
    """

    def _suffixes(self, mirror: MaildirMirrorRepository) -> list[str]:
        """Flag suffix per stored message, '' for a bare (unflagged) name."""
        out: list[str] = []
        for sub in ("new", "cur"):
            directory = mirror._root_dir / sub  # noqa: SLF001
            if not directory.exists():
                continue
            out += [
                name.partition("!2,")[2] if "!2," in name else ""
                for name in (p.name for p in directory.iterdir())
            ]
        return sorted(out)

    def test_an_ingested_message_carries_the_server_flags(self) -> None:
        # Stamped as the message is written: setting them afterwards
        # would mean a second pass over every message a sync ingests,
        # and the Maildir write is asynchronous so the file may not
        # even exist yet.
        service, _index, mirror, _session = _setup(
            server_folders={
                "INBOX": {
                    1: (
                        "<seen@example.com>",
                    ...
                    2: (
                        "<new@example.com>",
                        frozenset(),
                        _make_raw_message("b", "<b@example.com>"),
                    ),
                }
            }
        )
        service.sync()
        self.assertEqual(self._suffixes(mirror), ["", "S"])

    def test_a_flag_pulled_from_the_server_reaches_the_mirror(self) -> None:
        service, _index, mirror, session = _setup(
            server_folders={
                "INBOX": {
                    1: (
                        "<m@example.com>",
        ...
        service.sync()
        self.assertEqual(self._suffixes(mirror), [""])

        # Another client marks it read and flagged on the server.  The
        # MODSEQ bump is what makes the folder leave the fast path; without
        # it the sync correctly decides nothing changed and does no FETCH.
        session.folders["INBOX"][1] = (
            "<m@example.com>",
            frozenset({MessageFlag.SEEN, MessageFlag.FLAGGED}),
            _make_raw_message("a", "<a@example.com>"),
        )
        session._bump_modseq("INBOX", 1)  # noqa: SLF001
        service.sync()
        self.assertEqual(self._suffixes(mirror), ["FS"])

    def test_the_storage_key_survives_the_flag_write(self) -> None:
        # The Maildir write renames the file; if that changed the key the
        # index row would point at nothing.
        service, index, mirror, session = _setup(
            server_folders={
                "INBOX": {
                    1: (
        ...
        self.assertIn(row.storage_key, mirror.list_messages(folder=folder))
        self.assertTrue(
            mirror.get_message_bytes(folder=folder, storage_key=row.storage_key)
        )
