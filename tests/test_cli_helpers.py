...
def test_bbdb_auto_sync_imports_newer_source_and_exports_copy(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    paths = _paths(tmp_path)
    paths.data_dir.mkdir()
    source = tmp_path / "address-book.bbdb"
    source.write_text("source", encoding="utf-8")
    os.utime(source, (100, 100))
    index = mock.MagicMock()
    contacts = [mock.sentinel.contact]
    index.list_all_contacts.return_value = contacts

    with (
        mock.patch.object(cli, "import_bbdb_contacts", return_value=(2, 1)) as importer,
        mock.patch("pony.bbdb.write_bbdb") as writer,
    ):
        cli._bbdb_auto_sync(source, index, paths)

    importer.assert_called_once_with(index=index, bbdb_path=source)
    writer.assert_called_once_with(contacts, paths.data_dir / "contacts.bbdb")
    assert float((paths.data_dir / ".bbdb_imported").read_text()) == 100
    assert "2 new, 1 updated" in capsys.readouterr().out


def test_bbdb_auto_sync_skips_unchanged_source_but_still_exports(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    paths.data_dir.mkdir()
    ...
    source.write_text("source", encoding="utf-8")
    os.utime(source, (100, 100))
    (paths.data_dir / ".bbdb_imported").write_text("100", encoding="utf-8")
    index = mock.MagicMock()
    index.list_all_contacts.return_value = []

    with (
        mock.patch.object(cli, "import_bbdb_contacts") as importer,
        mock.patch("pony.bbdb.write_bbdb") as writer,
    ):
        cli._bbdb_auto_sync(source, index, paths)

    importer.assert_not_called()
    writer.assert_called_once_with([], paths.data_dir / "contacts.bbdb")


def test_bbdb_auto_sync_tolerates_corrupt_marker_and_marker_write_error(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    paths.data_dir.mkdir()
    source = tmp_path / "address-book.bbdb"
    source.write_text("source", encoding="utf-8")
    (paths.data_dir / ".bbdb_imported").write_text("invalid", encoding="utf-8")
    index = mock.MagicMock()
    index.list_all_contacts.return_value = []

    with (
        mock.patch.object(cli, "import_bbdb_contacts", return_value=(0, 0)) as importer,
        mock.patch("pony.bbdb.write_bbdb"),
